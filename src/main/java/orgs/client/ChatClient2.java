...
public class ChatClient2 implements AutoCloseable {
            ...
            while ((serverResponseJson = in.readLine()) != null) {
                ...
                switch (commandcall) {
                    case "VIDEO_CALL_OFFER":
                        ...
                        // --- TODO: Display UI prompt to accept/reject ---
                        // Ensure UI operations are on the Event Dispatch Thread (EDT)
                        final int finalCallerId = callerId; // Variables used in lambda must be final or effectively final
                        ...
                        SwingUtilities.invokeLater(() -> {
                            ...
                            int choice = JOptionPane.showConfirmDialog(
                                    ...
                                    "Incoming Video Call", // Dialog title
                            ...
                            try {
                                ...
                                if (acceptCall) {
                                    ...
                                    // These will be used by the VideoCaptureThread (to send to) and VideoReceiverThread (to filter/expect from)
                                    ...
                                    System.out.println("Accepted call from " + finalCallerUsername + ". Initiating video stream...");
                                } else {
                                    System.out.println("Rejected call from " + finalCallerUsername + ".");
                                    // No need to start video threads if the call is rejected.
                                    // Ensure any temporary resources (like UI elements for the incoming call) are cleaned up.
                                }
                            } catch (IOException e) {
                    ...
                    case "VIDEO_CALL_ACCEPTED":
                        // Payload: {"callee_id": X, "callee_username": "Y", "callee_public_ip": "A.A.A.A", "callee_udp_port": B}
                    ...
                    case "VIDEO_CALL_REJECTED":
                        ...
                        stopVideoCallThreads(); // Clean up if any resources were started
                        break;
        ...
        while (currentUser == null) {
            ...
            if ("1".equals(authChoice)) {
                ...
                if (loginResponse != null && loginResponse.isSuccess()) {
                    ...
                    try {
                        udpSocket = new DatagramSocket(); // Let OS assign a free port
    ...
    private void displayCommands() {
        ...
        System.out.println("15  vidio call ");
    }
        ...
        try {
            switch (commandInput) {
                ...
                case "14": // New command to get a media file
                    ...
                    return;
                case "15":
                    System.out.println("Enter the clint id ");
    ...
    // ... (rest of ChatClient2 methods like login, sendRequestAndAwaitResponse, sendFileBytes, closeConnection)
    ...
    public void initiateVideoCall(String targetUserId) {
        try {
            // Get target user ID from server (you might have a GET_USER_ID command or client-side cache)
            // For simplicity, assume you know the target ID from context or a previous interaction
            // For now, let's use a dummy ID and assume you'd query the server for actual ID
            //int targetUserId = -1; // Placeholder: replace with actual user ID lookup
            // Find user by username using a server command if not cached
//
//            Response getUserResponse = sendRequestAndAwaitResponse(new Request(Command.GET_USER_PROFILE, Map.of("username", targetUsername)));
//            if(getUserResponse.isSuccess() && getUserResponse.getData() != null) {
//                Map<String, Object> userData = gson.fromJson(getUserResponse.getData(), new TypeToken<Map<String, Object>>(){}.getType());
//                targetUserId = ((Double) userData.get("id")).intValue();
//            } else {
//                System.err.println("Could not find target user: " + targetUsername);
//                return;
//            }

            // --- STUN/TURN/ICE would go here to discover public IP/port ---
            // For simple demo, use local IP or a manually configured public IP
            String myPublicIp = InetAddress.getLocalHost().getHostAddress(); // Or use a STUN client
            if (myPublicIp.equals("127.0.0.1") || myPublicIp.startsWith("192.168.")) {
                System.out.println("Warning: Using private IP " + myPublicIp+ "\n NAT traversal needed for external calls.");
            }
            ...
            System.out.println("Video call initiation request sent to server for user: " + "targetUsername");
        } catch (IOException e) {
    ...
    private void startVideoCallThreads() {
        ...
        // You'd also need a JFrame or JPanel to display the video frames
        ...
        // Pass videoLabel to VideoReceiverThread so it can update the image
        videoReceiverThread.setVideoDisplayLabel(videoLabel);
    }

    private void stopVideoCallThreads() {
        ...
        // (Assuming videoFrame reference is accessible, e.g., via a field)
    }

    public void endCurrentVideoCall() {
        if (remoteIp != null) { // Check if a call is active
            // Send END_VIDEO_CALL request to server, indicating the partner to end with
            Map<String, Object> payload = new HashMap<>();
            // You need to know your current call partner's ID here
            // This assumes activeVideoCalls or similar state is maintained client-side
